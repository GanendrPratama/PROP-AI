    ...
    async getAllAds(filters = {}) {
        let query = `
            SELECT 
                p.id as ad_id,
                1 as user_id,
                -- Use location as title fallback since CSV has no title
                p.location as title, 
                -- Use source or static text as description
                COALESCE(p.source, 'No description') as description,
                p.price,
                -- Default status if column missing or null
                'active' as status,
