...
export default function Calculation() {
	...
	const validate = () => {
		...
		if (!form.landSize || Number(form.landSize) <= 0) newErr.landSize = 'Land size is required'
		if (!form.buildingArea || Number(form.buildingArea) <= 0) newErr.buildingArea = 'Building area is required'
		if (!form.bedrooms || Number(form.bedrooms) < 0) newErr.bedrooms = 'Bedrooms is required'
		if (!form.bathrooms || Number(form.bathrooms) < 0) newErr.bathrooms = 'Bathrooms is required'
		setErrors(newErr)
						...
						<form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							<div>
								...
								<input
									...
									min="0"
									step="0.01"
									className={`w-full rounded-md border ${errors.landSize ? 'border-red-500' : 'border-gray-300'} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#395192]`}
								/>
								{errors.landSize && <p className="text-sm text-red-600 mt-1">{errors.landSize}</p>}
							</div>
							...
							<div>
								...
								<input
									...
									min="0"
									step="0.01"
									className={`w-full rounded-md border ${errors.buildingArea ? 'border-red-500' : 'border-gray-300'} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#395192]`}
								/>
								{errors.buildingArea && <p className="text-sm text-red-600 mt-1">{errors.buildingArea}</p>}
							</div>
							...
							<div>
								...
								<input
									...
									min="0"
									step="1"
									className={`w-full rounded-md border ${errors.bedrooms ? 'border-red-500' : 'border-gray-300'} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#395192]`}
								/>
							...
							<div>
								...
								<input
									...
									min="0"
									step="1"
									className={`w-full rounded-md border ${errors.bathrooms ? 'border-red-500' : 'border-gray-300'} px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#395192]`}
								/>
									...
									{extras.map((ex, idx) => (
										<div key={idx} className="grid grid-cols-12 gap-2">
											<input
												...
												className="col-span-5 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#395192]"
											/>
											<input
												type="text"
												value={ex.value}
												...
												className="col-span-6 rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#395192]"
											/>
											<button
												type="button"
												onClick={() => removeExtra(idx)}
												className="col-span-1 text-red-600 border border-red-200 rounded-md hover:bg-red-50"
												aria-label="Remove"
											>
												✕
											</button>
							...
							<div className="md:col-span-2 flex gap-3 justify-end pt-2">
								...
								<button
									type="submit"
									disabled={submitting}
									className="bg-[#8F333E] text-white font-semibold px-6 py-2 rounded-md hover:opacity-90 disabled:opacity-60"
								>
